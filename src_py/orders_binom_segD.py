...
#pd.crosstab(dat_rll_only.set_index('buyer_segment_cd').ix[["D"]].email, dat_rll_only.set_index('buyer_segment_cd').ix[["D"]].actiontype)
temp = dat_rll_only.set_index([dat_rll_only.buyer_segment_cd])
tempD = temp.ix[['D']]

tempD = tempD.set_index('time')[['email','actiontype','month','year','num_logins_orders','price','total_amt']]
d_actions_emails = pd.crosstab(tempD.email, tempD.actiontype)
d_actions_emails['fb'] =d_actions_emails.comment + d_actions_emails.like
# engage in FB, order and login
d_engagers_orders = d_actions_emails[(d_actions_emails.fb>0) & (d_actions_emails.Order>0) & (d_actions_emails.Login>0)]

# only fb engagers - no logins or orders
d_engagers_only = d_actions_emails[(d_actions_emails.fb>0) & (d_actions_emails.Order==0) & (d_actions_emails.Login==0)]
# fb and login - no orders - none
# login - no orders - none
# d_engage_login = d_actions_emails[(d_actions_emails.fb==0) & (d_actions_emails.Order==0) & (d_actions_emails.Login>0)]
# print d_engage_login.head()

# no fb - just orders
d_orders_only = d_actions_emails[(d_actions_emails.fb==0) & (d_actions_emails.Order>0)]


 # apply codes
action_codes_d = tempD.actiontype.map(action_codes)
tempD['actiontype_codes'] = action_codes_d.values
tempD = tempD.sort_index(ascending=False)
print "data with encoded actions", tempD.head()


# reset index to email
#email = tempD.groupby(['email',tempD.index.month])
email = tempD.groupby('email')
...
# fill in states matrix for all users - ie count number of users (GROUP D) who went from state 1-1, 1-2,..
# where states are:
# 1 - like
# 2 - comment
# 3 - login
...
orders_d = tempD[tempD.actiontype =='Order']
orders_d.index = pd.to_datetime(orders_d.index)
orders_d_timeline = pd.crosstab(orders_d.email,  orders_d.index.month)
orders_d_timeline = orders_d_timeline.reset_index()
orders_d_timeline.head()


# convert counts to 0 or 1's for each user
order_d_binom = orders_d_timeline.set_index('email').applymap(lambda x: x>0).applymap(lambda x: 1 if x else 0)
order_d_binom.head()
